...
class Board:
    ...
    def declare_frozen(self, piece: Piece):
        for p in piece.points:
            assert isinstance(self._grid[p.y][p.x], int)
        for p in piece.points:
    ...
    def update_piece_location(self, piece: Piece, old_points: List[MinoPoint]) -> None:
        for p in old_points:
            self._grid[p.y][p.x] = None
        self.add_piece(piece)

    def add_piece(self, piece: Piece) -> None:
        for p in piece.points:
            assert self._grid[p.y][p.x] is None
        for p in piece.points:
            self._grid[p.y][p.x] = piece.colour_code
        ...
        for row in self._grid[self._padding:]:
            ...
            for x in row:
                if x is None:
                    ...
                    row_res.append(PIECE_COLOURS[x - 1].format(MARKER))
